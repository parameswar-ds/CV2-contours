    ...
    #     6
        ...
        for i in range(0, len(contours)):
            ...
            if (area > 0):
                ...
                if (w>150  and area>22000 and h>200 and h<500):
                    im = cv2.rectangle(src, (x, y), (x + w, y + h), (0, 255, 0), 2)
                    cv2.putText(src, str(i), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
                    print("{} : {} : {} : {} :{} : {}".format(c,i, area,extent, w, h))
                    ROI = src[y:y + h, x:x + w]
                    dsize = (50,100)
                    output = cv2.resize(ROI, dsize)
                    cv2.imwrite('/home/ram/Desktop/Anukai solutions/projects/automatic number plate detection/bike_samples/re/{}_{}_ROI_{}.png'.format(c,i,ROI_number), output)
                    ROI_number += 1
                    l.append(area)
    c+=1
print("complwted")
